...
- **Портативность.** Одна папка, без установщика, без записи в реестр.
  Настройки лежат рядом в `settings.json`. Скопировал папку — работает.
...
- ffmpeg — скачивается отдельным скриптом (см. «Установка»).
...
win64, GPL). Программа сама медиа не декодирует — она вызывает
`ffmpeg`/`ffprobe`/`ffplay` как внешние процессы. ffmpeg распространяется
